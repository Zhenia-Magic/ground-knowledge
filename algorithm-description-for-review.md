...
nEff answers "how many independent looks support this position," **not** "how good is each look." One decisive randomized trial is a single root; seven independent anecdotes are seven roots. The metric is deliberately an *independence audit* that composes with — and does not replace — per-study quality appraisal (GRADE-style risk-of-bias and similar), and it should be read next to the evidence-type, confidence, and method-class displays, never alone as a settledness score. It is also now shown *with* each position's evidence-type mix (§14.3), precisely so a reader never mistakes a high root-count for high quality — one decisive trial (nEff 1) can outweigh seven weak studies (nEff 7). **Honest caveat on the two halvings in §6.4:** the first (a root known only via secondary sources) grades *provenance strength* — how directly the knowledge base instantiates a root — which is genuinely not study quality. The second (a root backed only by weakly-applicable evidence, e.g. a different population) is a *limited external-validity* judgment, which **is** quality-adjacent; so the claim "nEff contains no quality judgment" is not strictly true, and we prefer to name that than to pretend the second halving is pure provenance. Broader weighting of roots by elicited quality remains an open extension, named rather than smuggled in.
...
**Scope of that claim, stated honestly.** "Domain-general" here means *general across empirical-causal disputes* — questions where a position is ultimately backed by underlying data-generating processes (datasets, cohorts, experiments, observations) and where **shared data is a reason to discount, not to add.** It is **not** a universal dispute-mapper. In a **legal** dispute the "roots" would be precedents, and weight of authority is partly *cumulative* — twenty rulings citing one precedent are more authoritative, not one look — so collapsing echo would give the wrong answer. In a **mathematical** dispute validity is not a counting exercise at all; a lemma used by ten theorems is not "0.1 independent." Even within empirical fields, the primary/secondary tier table and the animal/in-vitro halving are biomedicine-shaped defaults; a physics or economics dispute needs those *re-specified*, which is more than a vocabulary swap. So "new domain adds vocabulary, never code" holds cleanly for empirical-causal questions and degrades outside them — an honest boundary, not a universal claim.
...
Defenses built into this layer: duplicate refusal (same URL, or same title+year even under a different URL), alias resolution (the same resource submitted under different names collapses to one entity, so nobody can fake independence by renaming), and off-topic refusal (a real but irrelevant document is kept out of every metric rather than padding a position). Crucially, a refusal is **recorded, not silent** — the refused title and reason are written to the knowledge base and surfaced in the report, so a *wrongly* refused source is visible and reversible rather than invisibly suppressed (the inverse failure a false-balance tool must avoid).
...
- **Distribution** — naive share of *sources* per position, and, alongside it, the independence-weighted version (each position resized by its effective independent bases). Seeing a position contract between the two views is the central point made visible. The weighted view is explicitly labelled as measuring *independence, not quality*, and each position's **evidence-type mix** (how many primary vs secondary, and the design breakdown) is shown beside its independent-bases count — so a position with seven weak studies (nEff 7) is never silently read as better-supported than one with a single decisive trial (nEff 1).

...
python3 eval/run_benchmark.py --require-live-baseline --check-results >/dev/null && echo "  benchmark PASS"

echo "== 4/6  support-edge migration completeness =="
python3 scripts/migrate_edge_admissions.py --check >/dev/null && echo "  edge admissions current"

echo "== 5/6  generated source-inventory freshness =="
python3 scripts/source_inventory.py --check >/dev/null && echo "  source inventory current"

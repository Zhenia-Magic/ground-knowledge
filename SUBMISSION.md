...
**Comparative recall (honest).** The benchmark now also scores the ChatGPT / Claude reports against the *same* gold, as a keyword-recall proxy (with paraphrase synonyms, so prose is credited fairly). The result is **near-parity** — a good deep-research report surfaces the same positions, roots, and cruxes GK does. The informative part is the **losses**: both baselines surface the eggs *biomarkers* crux and the black-hole *cosmic-ray* crux that GK's detector misses (the two known sub-threshold factors). We report those, not just wins.
